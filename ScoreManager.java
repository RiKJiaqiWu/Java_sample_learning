        ...
        while (cont) {
            ...
            switch (oprtId) {
                case 1:
                    ...
                    int [] bestofYearText = new int[10];
                    int bestOfYearCount = 0;
                    // 循环指定年份的成绩，找出最好的成绩
                    // 已解决若有多年成绩一样的问题
                    for (int i = 1; i < totalScoreCount; i++) {
                        if (scores[year][bestOfYearScoreId] == scores[year][i]) {
                            bestOfYearCount++;
                            bestofYearText[i-1] = i;
                        } else if(scores[year][bestOfYearScoreId] < scores[year][i]) {
                            bestOfYearScoreId = i;
                        }
                    }
                    if(bestOfYearCount != 0){
                        System.out.println("第" + (year + 1) + "年成绩最好的科目为" + names[bestOfYearScoreId] + "，成绩为" + scores[year][bestOfYearScoreId] + "。");
                        for(int i = 0;i < bestOfYearCount; i++){
                            bestOfYearScoreId = bestofYearText[i];
                            System.out.println("第" + (year + 1) + "年成绩最好的科目为" + names[bestOfYearScoreId] + "，成绩为" + scores[year][bestOfYearScoreId] + "。");
                        }
                    }else {
                        System.out.println("第" + (year + 1) + "年成绩最好的科目为" + names[bestOfYearScoreId] + "，成绩为" + scores[year][bestOfYearScoreId] + "。");
